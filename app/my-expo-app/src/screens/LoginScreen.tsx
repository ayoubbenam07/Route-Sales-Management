        ...
        >
          ...
          <View className="rounded-3xl border border-slate-100 bg-white p-8 shadow-sm">
            ...
            <View className="mb-6 flex-row rounded-xl bg-slate-100 p-1">
              {(["ADMIN", "BUYER"] as Role[]).map((r) => (
                <TouchableOpacity
                  key={r}
                  onPress={() => setRole(r)}
                  className={`flex-1 rounded-lg py-2.5 ${role === r ? "bg-white" : ""}`}
                >
                  <Text
                    className={`text-center text-sm font-semibold ${
                      role === r ? "text-indigo-600" : "text-slate-500"
                    }`}
                  >
                    {r === "ADMIN" ? t("auth.adminTab") : t("auth.buyerTab")}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
